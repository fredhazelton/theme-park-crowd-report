...
|----------|-------------|
| `/api/park-wti-distributions` | Per-park WTI percentiles (p5, p25, median, p75, p95) for Benedictus color scaling |
...
**Per-park color scaling:** The stream dashboard fetches `/api/park-wti-distributions` at load. KPI cards and WTI lollipops use per-park percentiles for Benedictus colors (p5→deep blue, p25→blue, median→lavender, p75→pink, p95→red). Data from `state/park_wti_distributions.json` (generated by `compute_park_wti_distributions.py` in the pipeline).
