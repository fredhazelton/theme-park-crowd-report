...
```
Fred has an idea or spots a need
         ↓
Wilma captures it → adds to task board or TPCR priorities
         ↓
Wilma routes to the right person:
  • Code/pipeline → Bam-Bam (with Barney QA if methodology)
  • Content → Betty (with Pebbles for visuals)
  • Research → Barney
  • Design → Pebbles
         ↓
Agent does work → domain-matched QA checks it
         ↓
Wilma reviews → ships or escalates to Fred
         ↓
Fred reviews only: go/no-go gates, pricing, external comms, strategic calls
```

**Current reality:** Most work is Fred ↔ Bam-Bam for pipeline, Fred ↔ Wilma for everything else. The formal routing kicks in as we enter Phases 4-5.
...
| Weekly | Progress summary — what shipped, what's next, any blockers | Wilma compiles → Fred reviews |
