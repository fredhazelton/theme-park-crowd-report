...
**Date:** Feb 18, 2026  
**Priority:** 🔴 URGENT — START TONIGHT  
**Deadline:** Models retrained + forecasts generated by Feb 19 morning. We need 2 days of live observations paired with new forecasts for A/B comparison by Feb 21.  
...
**🕐 TIMELINE:**
- **Tonight (Feb 18):** Retrain models with 5 temporal features on synthetic actuals + real actuals. Save as new model files (e.g., `model_actuals_first_v1.json`) alongside existing V2 models — do NOT overwrite production models yet.
- **Feb 19 morning:** Generate forecasts using new models. Archive them separately (e.g., `forecast_actuals_first_YYYY-MM-DD.parquet`).
- **Feb 19-20:** Live observations accumulate against both old and new forecasts.
- **Feb 21:** Compare accuracy — old models vs Actuals-First. If Actuals-First wins (or ties), it becomes production.

**Memory note:** The server OOM'd today when loading 90M synthetic pairs for training. You may need to:
- Train in chunks (by park) instead of loading all 90M rows at once
- Or subsample synthetic actuals (e.g., 1 in 10) — still gives 9M+ training examples
- Or use DuckDB for out-of-core training data prep before feeding to XGBoost
